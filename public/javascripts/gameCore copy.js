...
$(function() {
    ...
    $(document).on("keydown", function (event) {
        ...
        if (gameStarted && contador < ROUNDS && keyMap[event.key.toLowerCase()]) {
            getTime()
            contador++
            checkCorrect(event.key.toLowerCase())
            updateGame()
            updateUI()
        }
    });
...
async function showResults() {
    ...
    if(levelUp) {
        ...
        setTimeout(() => $("#experienceBar").css("opacity",0) , 2600);
        setTimeout(() => $("#experienceBar").css("width","0%") , 2600);
        setTimeout(() => $("#experienceBar").css("opacity",1) , 3200);
        setTimeout(() => $("#experienceBar").css("width",experiencePercentage + "%") , 3200);
    } else {
